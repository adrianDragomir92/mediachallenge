...
@section('title', 'Contact | Media Challenge')
@section('meta_description', 'Trimite-ne un mesaj pentru social media, site-uri și SEO. Răspundem rapid cu un plan potrivit brandului tău.')
...
@section('content')
    ...
    <section class="max-w-6xl mx-auto px-6 pb-16 grid gap-10 lg:grid-cols-2">
        <form class="rounded-[32px] border border-white/10 bg-white/5 p-8 shadow-glow space-y-6">
            <div>
                <label for="nume" class="text-sm text-white/60">Nume și prenume</label>
                <input id="nume" type="text" placeholder="Ana Marinescu" class="mt-2 w-full rounded-2xl bg-midnight/60 border border-white/10 px-4 py-3 text-white focus:border-neon focus:outline-none" />
            </div>
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    <label for="email" class="text-sm text-white/60">Email</label>
                    <input id="email" type="email" placeholder="email@brand.ro" class="mt-2 w-full rounded-2xl bg-midnight/60 border border-white/10 px-4 py-3 text-white focus:border-neon focus:outline-none" />
                </div>
                <div>
                    <label for="telefon" class="text-sm text-white/60">Telefon</label>
                    <input id="telefon" type="tel" placeholder="07xx xxx xxx" class="mt-2 w-full rounded-2xl bg-midnight/60 border border-white/10 px-4 py-3 text-white focus:border-neon focus:outline-none" />
                </div>
            </div>
            <div>
                <label for="servicii" class="text-sm text-white/60">Servicii dorite</label>
                <select id="servicii" class="mt-2 w-full rounded-2xl bg-midnight/60 border border-white/10 px-4 py-3 text-white focus:border-neon focus:outline-none">
                    <option value="">Selectează</option>
                    <option>Social Media</option>
                    <option>Creare site-uri</option>
                    <option>SEO</option>
                    <option>Pachet complet</option>
                </select>
            </div>
            <div>
                <label for="mesaj" class="text-sm text-white/60">Mesaj</label>
                <textarea id="mesaj" rows="4" placeholder="Spune-ne câteva detalii despre afacerea ta."
                    class="mt-2 w-full rounded-2xl bg-midnight/60 border border-white/10 px-4 py-3 text-white focus:border-neon focus:outline-none"></textarea>
            </div>
        ...
        <div class="rounded-[32px] border border-white/10 bg-white/5 p-8 space-y-6">
            <div>
                ...
                <h2 class="text-2xl font-semibold">București · Cluj</h2>
                <p class="text-white/70 mt-2">Ne întâlnim fizic sau online. Lucrăm cu branduri din toată țara.</p>
            </div>
